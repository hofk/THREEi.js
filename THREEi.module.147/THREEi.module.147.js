// THREEi.module.js *** to r147 *** 

/**
 * @author hofk / https://threejs.hofk.de/
*/

import { BufferAttribute } from '../jsm/three.module.147.js';
...
function createSphereWithHoles( arg1, arg2 ) {
	...
	g = this;  //  BufferGeometry() - geometry object from three.js
    ...
    if( typeof arg1 === 'number' ) { // variant detail, holes are optional,
		...
		g.detail = arg1; // g.detail = detail; // count of triangles for half a great circle  !!!!!!! 
		g.holes = arg2 !== undefined ? arg2 : []; // g.holes = holes !== undefined ? holes : []; // optional !!!!!!
...
function buildSphereWithHolesObj( ) {
	...
	g.setIndex( new BufferAttribute( g.indices, 1 ) );
	g.setAttribute( 'position', new BufferAttribute( g.positions, 3 ) );
...
function buildSphereWithHoles( ) {
	...
	function makeCircularHole( i ) {
		...
		let r = count / g.detail / 2; // radius cutting circle
...
function buildImplicitSurface( ) {
	...
	function triangulation( isf, dx, dy, dz, xs, ys, zs, d, e, fc, pc ) {
		...
		g.setIndex( new BufferAttribute( g.indices, 1 ) );
		g.setAttribute( 'position', new BufferAttribute( g.positions, 3 ) );
		g.setAttribute( 'normal', new BufferAttribute( g.normals, 3 ) );
...
export {
     createSphereWithHoles, buildSphereWithHolesObj, buildSphereWithHoles,
     createCylinderWithHoles, buildCylinderWithHoles,
     createInnerGeometry, buildInnerGeometry,
     createImplicitSurface, buildImplicitSurface     
};